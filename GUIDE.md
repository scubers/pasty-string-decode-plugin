...
**配套文档**（均随 SDK 包 [`@pasty/plugin-sdk`](https://www.npmjs.com/package/@pasty/plugin-sdk) 发布）：

- **`API.md`** — 由 `protocol/plugin/src/catalog.ts` 自动生成的 API 真相源（capability / host event / 类型一览）。运行 `cd protocol/plugin && npm run codegen` 会同步刷新这份文件。**有冲突时以 API.md 为准**。
- **`README.md`** — SDK 公共符号速查（导出、运行时入口、UI 入口）。
- **`SPECIFICATION.md`** — SDK 形状规则、命名约定、扩展 capability 的 PR 流程。
...
UI 侧拿到的所有宿主状态遵守三种形状之一（详见 [`@pasty/plugin-sdk`](https://www.npmjs.com/package/@pasty/plugin-sdk) 包内 `SPECIFICATION.md` 第 1 章）：
...
SDK 暴露两个 subpath。一边导出的符号**完全由 codegen 生成**，列在包内 `API.md`（[`@pasty/plugin-sdk`](https://www.npmjs.com/package/@pasty/plugin-sdk)）。
...
**每个方法的精确 payload / response 形状以 SDK 包内 `API.md`（[`@pasty/plugin-sdk`](https://www.npmjs.com/package/@pasty/plugin-sdk)）为准。** GUIDE 不再重复 23 个 capability 的完整签名表——那是 codegen 的责任。
...
入参 `PluginDetectorInput`（详见 SDK 包内 `API.md` §8）：
...
A：见 SDK 包内 `SPECIFICATION.md`（[`@pasty/plugin-sdk`](https://www.npmjs.com/package/@pasty/plugin-sdk)）第 3 章。简而言之：改 `protocol/plugin/src/catalog.ts` → 跑 `npm run codegen` → 在宿主端实现新方法 → 添测试。SDK 表面由 codegen 自动暴露。

---

## 10. 完整对照表

要查具体某个 capability / host event / 类型的精确形状（payload、response、wire path、context），请直接看 SDK 包内 `API.md`（[`@pasty/plugin-sdk`](https://www.npmjs.com/package/@pasty/plugin-sdk)）——它是从 `protocol/plugin/src/catalog.ts` 由 codegen 直出，**保证与代码一致**。本 GUIDE 描述模式与约定，API.md 描述每个符号。
